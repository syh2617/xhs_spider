...
def save_in_user_detail(data):
    ...
    cursor = conn.cursor()
    # 达人个人信息表
    insert_sql = ("INSERT INTO test_user_details VALUES (%s, %s, %s, %s, %s, "
                  "%s,%s, %s, %s, %s,"
                  "%s,%s, %s, %s, %s,  %s)")
...
def kafka_consumer():
    ...
    try:
        while True:
            ...
            cookie = account_Pool[num % 5]
            #test暂用cookie： 'nickname': '安东尼4235', 'way': '手机号4235', 'browser': 'QQ',
...
def test(id_list):
    ...
    try:
        while True:
            ...
            cookie = account_Pool[num % 5]
            # test暂用cookie： 'nickname': '飒
            cookie = account_Pool[8]
            cookies_nickname = cookie['nickname']
            cookies_str = cookie.get('cookies_str')
...
if __name__ == '__main__':
    # #daily_user_crawler()
    filename = ('D:\\PycharmProjects\\XHS_240808\\test_user\\15w_user_link.csv')
    # a1='15w_user_link.csv'
    id_list = read_csv(filename)
    id_list=id_list[3700:10000]
    print("start! run!")
    # print(id_list)
    # #print(id_list[:6])
    test(id_list)
