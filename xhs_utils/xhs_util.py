...
try:
    js = execjs.compile(open(r'../static/xhs_xs_xsc.js', 'r', encoding='utf-8').read())
except:
    js = execjs.compile(open(r'static/xhs_xs_xsc.js', 'r', encoding='utf-8').read())
...
def generate_x_xray_traceid(j15):
    a15 = a(j15)
    format_str = "{:016x}{:016x}".format(a15[0], a15[1])
    # print('len:',len(format_str))
    return format_str
def a(j15):
    f2696b = random.randint(0, 2 ** 31 - 1)
    return [(j15 << 23) | (f2696b & 8388607), random.randint(-9223372036854775808, 9223372036854775807)]
...
def generate_headers(a1, api, data=''):
    ...
    # 20241015
    # headers['x-xray-traceid'] = generate_x_xray_traceid(int(xt))
    if data:
